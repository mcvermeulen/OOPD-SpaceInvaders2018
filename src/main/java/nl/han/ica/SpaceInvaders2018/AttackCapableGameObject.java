...
public abstract class AttackCapableGameObject extends DestroyableGameObject {
    ...
    public AttackCapableGameObject(Sprite sprite, int totalFrames, float x, float y, int sWidth, int sHeight) {
    ...
    public void generateLaser(SpaceInvaders world, boolean friendly) {
        Laser laser = new Laser (friendly);
        projectiles.add(laser);
        world.addGameObject(laser, getX() + (getWidth()/2), getY() /*+ getHeight()*/); // laser loopt nog over cannon heen // TODO
    }
